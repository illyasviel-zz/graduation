...
wav_len=sta:0.001:en;
wav_len=wav_len.*(10^(-9));
n_eff=1.485;

% FBG_periods=521.4*10^(-9);
FBG_wavlen=1558*1e-9;

FBG_periods=FBG_wavlen/(2*n_eff)
...
c=Cg;
% lambda0=FBG_wavlen;
wav_len1=wav_len-1558.55;
T1=(1+cos(4*pi/1e17*n_eff.*wav_len/(lambda0^2).*(delta_L0+(wav_len-FBG_wavlen)/c)));
 T_lambda=1/2*R.*(1+cos(4*pi/5e12*n_eff.*wav_len1/(lambda0^2).*(delta_L0+(wav_len1-FBG_wavlen)/c)));

%T_lambda=1/2*R.*(1+cos(4*pi/1e16*n_eff.*wav_len/(lambda0^2).*(delta_L0+(wav_len-FBG_wavlen)/c)));
% %figure
%plot(wav_len,T1)
...
xlabel('wavelength(nm)');
ylabel('Amplitude');
%fai_lambda=3e8./wav_len.^2*Q(2);
%t=wav_len.*fai_lambda;
%t0=FBG_wavlen.*fai_lambda
%T_time=1/2*R.*(1+cos(4*pi/1e12*n_eff.*wav_len1/(lambda0^2).*(delta_L0+(wav_len1-FBG_wavlen)/c)))
%fai_lambda=3e8./wav_len.^2*Q(2);
%t=wav_len.*fai_lambda;
%plot(t,T_lambda)
%xlim([5.96e5,6.02e5])
